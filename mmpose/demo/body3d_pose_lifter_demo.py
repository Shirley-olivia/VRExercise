...
from mmpose.structures import (PoseDataSample, merge_data_samples,
                               split_instances, JointAngle, JointAngleVelocities, JointAngleAcceleration, YawAngle)
...
def parse_args():
    ...
    parser.add_argument(
        '--show-interval', type=int, default=0.002, help='Sleep seconds per frame')
    parser.add_argument(
        '--thickness',
        type=int,
        default=1,
...
def main():
    ...
    if args.save_predictions:
        ...
        args.tmp_save_path = f'{args.output_root}/results_' \
                              f'{os.path.splitext(os.path.basename(args.input))[0]}_1.json'
    ...
    if input_type == 'image':
        ...
        if args.input == 'webcam':
            ...
            while True:
                ...
                ret_color, frame = capture.get_color_image()
                ...
                # 关节角度
                joint_angles = JointAngle(pred_3d_instances)
                # for joint_name, angle in joint_angles.items():
                #     print(f"{joint_name}: {angle} degrees")

                # 关节角速度
                joint_angle_velocities = JointAngleVelocities(joint_angles, last_angles)
                # for joint_name, velocity in joint_angles.items():
                #     print(f"{joint_name}: {velocity}

                # 关节角加速度
                joint_angle_accelerations = JointAngleAcceleration(joint_angle_velocities, last_velocities)

                last_angles = joint_angles
                last_velocities = joint_angle_velocities

                # 偏转角
                yaw_angles = YawAngle(pred_3d_instances)
                ...
                new_data = dict(
                    ...
                    joint_angle_accelerations=joint_angle_accelerations,
                    yaw_angles=yaw_angles,
                )
                # print(new_data)

                if args.save_predictions:
                    # with open(args.pred_save_path, 'a') as f:
                    #     json.dump(
                    #         dict(
                    #             instance_info=new_data),
                    #         f,
                    #         indent='\t')
                    #     f.write('\n')
                    #     f.write('#')
                    ...
                    if not os.path.exists(args.pred_save_path):
                        print("create")
                        with open(args.tmp_save_path, 'w') as f:  # Use 'w' mode to create and write if the file doesn't exist
                            json.dump(
                                dict(instance_info=new_data),
                                f,
                                indent='\t'
                            )
                            # f.write('\n')
                            # f.write('#')
                            # f.write('\n')
                        os.rename(args.tmp_save_path, args.pred_save_path)
                    else:
                        print(f"The file '{args.pred_save_path}' already exists. Skipping writing process.")
